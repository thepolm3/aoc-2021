...
impl<const N: usize, const M: usize> SeaFloor<N, M> {
    ...
    fn basin(&self, x:usize, y:usize) -> HashSet<(usize, usize)> {
        ...
        loop {
            let neighbors: HashSet<(usize, usize)> = basin.iter()
            .flat_map(|(x,y)| {
                self.neighbors(*x, *y)
            })
            .unique()
...
fn main() -> Result<()>{
    ...
    let sea_floor: Vec<u32> = input.lines()
        .flat_map(|line|
            line.chars()
                .map(|char|
                    char.to_string().parse::<u32>()
                )
        ).try_collect()?;
    ...
    let mins = (0..10000usize).into_iter()
        .map(|pos| (pos%100,pos/100))
        .filter(|(x,y)| 
            sea_floor.is_minimum(*x, *y)
        ).collect_vec();
    
    let part1 = mins.iter().map(|(x,y)| {
            sea_floor.get(*x,*y) + 1
        }).sum::<u32>();
    let part2 = mins.iter()
        .map(|(x,y)| {
            sea_floor.basin(*x,*y).len()
        })
        .sorted()
        .rev()
        .take(3)
